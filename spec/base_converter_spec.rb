  ...
  describe '#value_from_decimal' do
    context "custom base 62" do
      it 'generates a base62 based on a a given value' do
        base62_converter = BaseConverter.new([*'0'..'9', *'a'..'z', *'A'..'Z'], 62)
    ...
    context "binary" do
      let(:binary_symbols) { ['0', '1'] }
      it 'sets 0, 1 and 10 to their base 2 equivalents' do
        base2_converter = BaseConverter.new(binary_symbols, 2)
      ...
      it 'converts unsigned short integer max to binary' do
        expect(BaseConverter.new(binary_symbols, 2).value_from_decimal(65535)).to(eq("1111111111111111"))
      end
    end
    ...
    context 'hexidecimal' do
      ...
      it 'converts 1 through 10 to their base16 equivalent' do
        base_converter = BaseConverter.new(hex_symbols, 16)
      ...
      it 'converts unsigned short integer max to hex' do
        expect(BaseConverter.new(hex_symbols, 16).value_from_decimal(65535)).to(eq("FFFF"))
      end
    end
    ...
    context "base 60" do
      ...
      it "converts decimal to sexagesimal values" do
        expect(BaseConverter.new(sexagesimal_symbols, 60).value_from_decimal(60)).to(eq("10"))
      end
    end
  end
end